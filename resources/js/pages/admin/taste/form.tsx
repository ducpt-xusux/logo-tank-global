...
export default function TasteForm({ item }: TasteFormProps) {
    const { data, setData, patch, errors, processing, post } = useForm<Taste>(
        item ?? ({} as Taste),
    );
    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        if (item) {
            patch(route("admin.taste.update", { taste: item }), {
                preserveScroll: true,
                onSuccess: () => {
